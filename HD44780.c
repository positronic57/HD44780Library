...
void LCDInit4(TSHD44780 *pHD44780,volatile uint8_t *HD44780_CMD_PORT,volatile uint8_t *HD44780_DATA_PORT,uint8_t HD44780_RS,uint8_t HD44780_E)
{
	...
	*(pHD44780->HD44780_CMD_PORT) &= ~(_BV(HD44780_RS) |_BV(HD44780_E)); //Set the control bus PINs to logical 0
	*(pHD44780->HD44780_CMD_PORT-1) |= (_BV(HD44780_RS)|_BV(HD44780_E));  // Define the control bus PINs as outputs
	...
	*(pHD44780->HD44780_CMD_PORT) &= ~(_BV(HD44780_E));
    ...
    LCDSendCommand4(pHD44780,0x28);
	LCDSendCommand4(pHD44780,CURSOR_ON); 
	LCDSendCommand4(pHD44780,0x06);
